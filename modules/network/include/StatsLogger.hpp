...
private:
    static NetworkStatsLogger* _instance;
...
public:
    static NetworkStatsLogger* instance();
    ...
    constexpr void log(PacketType message_type, bool is_sent, const Address& target, const ByteBuffer& content) {
        std::span<uint8_t> span = allocate_memory_for_buffer(content.writing_head());
        memcpy(span.data(), content.data().data(), content.writing_head());
    ...
    constexpr void log_receive(
        ...
        m_incoming.push(content.writing_head());
    }
    ...
    constexpr void log_send(
        ...
        m_outgoing.push(content.writing_head());
    }
